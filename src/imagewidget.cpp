...
ImageWidget::ImageWidget(QGraphicsView *instance) : instance{instance} {
    graphicsScene = new QGraphicsScene;//要用QGraphicsView就必须要有QGraphicsScene搭配着用
    init();
}
...
//            cout<<"右边框"<<endl;
            direction = dir_right;
            instance->setCursor(Qt::SizeHorCursor);
        } else if (QRectF(roirect.topLeft().x() + EDGPADDING, roirect.topLeft().y() + EDGPADDING,
                          roirect.width() - 2 * EDGPADDING, roirect.height() - 2 * EDGPADDING).contains(mousep)) {
            // roi中间
//            cout<<"roi中间"<<endl;
            direction = dir_mid;
...
void ImageWidget::setQGraphicsViewWH(int nwidth,
...
void ImageWidget::setImage(QPixmap *pixmap) {
    m_pix = *pixmap;
}


void ImageWidget::graphics(QImage &image, string path) {
    img_path = path;
    roirect = QRectF(0, 0, 0, 0);
    ...
    instance->setSceneRect(QRectF(-(nwith / 2), -(nheight / 2), nwith,
...
void ImageWidget::moveRect(const QPointF &mousePoint) {
...
void ImageWidget::moveRectLine(QGraphicsSceneMouseEvent *event, EnumDirection dir) {
    QRectF tmp_rect;
    auto mousep = event->pos();
...
void ImageWidget::scaleRect(QGraphicsSceneMouseEvent *event, EnumDirection dir) {
    QRectF tmp_rect;
    auto mousep = event->pos();
